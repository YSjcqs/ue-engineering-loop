...
- **显式 `-log=<path>` 与 `-ExecCmds` 组合在 5.8 不生成日志文件** → 使用**默认日志**。脚本在跑前把旧日志**改名**为 `<Project>.log.prev_<时间戳>`（**绝不删除**，保留最近 5 份，连续运行不会互相覆盖）。
...
| 退出码 | 含义 | 处置 |
|---|---|---|
| `0` | 全部通过（Success > 0 且 Fail == 0 且 editor exit 0） | ✅ 通过 |
| `1` | 有测试失败 | ❌ 修 bug |
| **`2`** | **零测试执行**（Spec 名错 / Spec 不存在） | ❌ **绝不能当通过** |
| `3` | 无日志产出（引擎没起来） | ❌ 查引擎 |
| `4` | 参数错误（项目/引擎路径无效） | ❌ 修参数 |
| `5` | 超时 | ❌ 查卡点 |
...
> **纪律**：不可以用「没报错」「引擎退出了」「日志有了」来推断测试通过——**只看 `Executed` 计数与退出码**。
